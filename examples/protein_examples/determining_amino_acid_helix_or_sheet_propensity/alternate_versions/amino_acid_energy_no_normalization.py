...
his program assumes the file contains a sequence of letters, and
...
def main():
    ...
    prior__helices = (float(NAA_hel)/float(NAA_tot))
    P = {}
    deltaP = {}
    for c in type_count:
        conditional_probability__c = float(type_count[c])/type_count_tot
        joint_probability__c_helix = conditional_probability__c * prior__helices
        P[c] = joint_probability__c_helix
        # Suppose x = 
        # is the probability of encountering a certain letter (c)
        # in a huge list of N letters (in this example NAA_tot).
        # It's easy to show that the uncertainty in the estimate for 
        # the probability is sqrt(x*(1-x)/N) for large N.
        deltaP[c] = math.sqrt((P[c]*(1.0-P[c])) / type_count_tot)
    ...
    T  = 300.0
    sys.stdout.write('\nProbabilities for each type (sum = '+str(prior__helices)+'):\n')
    #sys.stdout.write('C AA Horovitz1992 ONeil1990 ln(Pevo(AA)/P(AA))*kB*T delta(ln(P(AA)))*kB*T P(AA) deltaP Pevo(AA)\n')
    sys.stdout.write('C AA P(AA) deltaP Pevo(AA) ln(Pevo(AA)/P(AA))*kB*T delta(ln(P(AA)))*kB*T ONeil1990 Horovitz1992 Blaber44_1993 Myers1997pep Myers1997pro\n')
    ...
    for ener_pair in ener_sorted:
        ...
        sys.stdout.write((' %5.3f'%((ener[c]-ave_ener)*kB*T)) + (' %5.3f'%(delta_ener[c]*kB*T)))
